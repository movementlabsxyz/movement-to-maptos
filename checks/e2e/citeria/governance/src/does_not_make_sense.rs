...
fn unprotected_entry_functions() -> Vec<(ModuleId, Identifier)> {
	...
	.map(|(module_str, fn_str)| {
		(ModuleId::from_str(module_str).unwrap(), Identifier::new(fn_str).unwrap())
	})
	.collect()
}
...
impl Criterionish for GovernanceCheck {
	fn satisfies(&self, maptos_executor: &MovementAptosExecutor) -> Result<(), CriterionError> {
		...
		for (module_id, function_name) in unprotected_entry_functions() {
			let entry_fn =
				EntryFunction::new(module_id.clone(), function_name.clone(), vec![], vec![]);
			...
			match result {
				Ok(_) => violations.push(format!(
					"{}::{} is callable and lacks signer check",
					module_id, function_name
				)),
				Err(err) => {
					eprintln!(
						"{}::{} simulation failed as expected: {:?}",
						module_id, function_name, err
					);
				}
			}
		}
		...
		if !violations.is_empty() {
			return Err(CriterionError::Unsatisfied(
				format!("The following functions are callable:\n{}", violations.join("\n")).into(),
			));
		}
