...
impl Migrationish for Migrate {
	async fn migrate(
		...
		let indexer_schema_db = open_internal_indexer_db(db_dir.clone(), &Default::default())
			.map_err(|e| MigrationError::Internal(e.into()))?;
		let mut indexer_db_config = InternalIndexerDBConfig::default();
		indexer_db_config.enable_statekeys = true;
		let indexer_db = InternalIndexerDB::new(Arc::new(indexer_schema_db), indexer_db_config);
